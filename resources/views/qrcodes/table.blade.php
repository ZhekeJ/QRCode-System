<div class="table-responsive">
    <table class="table" id="qrcodes-table">
        ...
        @foreach($qrcodes as $qrcode)
            ...
            <td> <a class="text-info" href="{{ route('qrcodes.show', [$qrcode->id]) }}">
               <b> {{ $qrcode->product_name }} </b></a></td>
            <td>{{ $qrcode->company_name }}</td>
            <td>{{ $qrcode->website }}</td>
            <td>{{ $qrcode->amount  }}R</td>
            <td>
               @if ($qrcode->status == 1)
               <i class="fa fa-check-square text-blue"></i>

               @else
               <i class="fa fa-window-close text-red"></i>
               @endif
                </td>
                <td>
